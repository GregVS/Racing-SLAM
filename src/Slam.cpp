...
{
    // First first frames
    auto maybe_result = init::find_initializing_frames(
        [this]() { return process_next_frame(); }, m_camera, *m_feature_extractor);
    if (!maybe_result) {
    ...
    // Triangulate points
    auto matches =
        m_feature_extractor->match_features(ref_frame->features(), query_frame->features());
...
void Slam::init_key_frame(Frame& frame)
{
    ...
    if (m_config.triangulate_points) {
        auto feature_matches =
            m_feature_extractor->match_features(last_key_frame->features(), frame.features());
        auto unmatched = features::unmatched_features(*last_key_frame, frame, feature_matches);
        auto points =
            triangulation::triangulate_points(*last_key_frame, frame, unmatched, m_camera);
        for (int i = 0; i < points.size(); i++) {
