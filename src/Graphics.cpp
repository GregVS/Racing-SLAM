...
Graphics::Graphics(int w, int h) : m_width(w), m_height(h) {
...
void draw_camera_poses(const std::vector<cv::Mat> &camera_poses) {
...
void Graphics::draw_scene(const std::vector<cv::Mat> &camera_poses, const Map &map) {
    process_input();
    ...
    draw_axes();
    draw_camera_poses(camera_poses);
...
void Graphics::mouse_callback(GLFWwindow* window, double xpos, double ypos) {
    ...
    if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
        // Pan camera
        graphics->m_camera_pos -= xoffset * 0.05f * glm::normalize(glm::cross(graphics->m_camera_front, graphics->m_camera_up));
