...
void run_viewer(Slam& slam, const Camera& camera)
{
    ...
    while (!visualization.has_quit()) {
        ...
        if (!visualization.wait_for_step()) {
            break;
        }
        slam.step();
    }
...
void write_point_cloud(const std::filesystem::path& path, const Slam& slam)
{
    ...
    for (const auto& point : slam.map()) {
        ...
        ply << p.x() << " " << p.y() << " " << p.z() << " " << int(c[0]) << " " << int(c[1]) << " " << int(c[2]) << " "
            << first << "\n";
    }
}
...
bool write_outputs(const Options& options, const Slam& slam, int seed, double seconds_per_frame)
{
    ...
    if (!trajectory::write_kitti(trajectory_path.string(), trajectory)) {
        std::cerr << "Failed to write trajectory: " << trajectory_path << '\n';
        return false;
    }

    std::ofstream meta(run_dir / (options.sequence + ".meta.yaml"));
    meta << "sequence: " << options.sequence << '\n';
    meta << "run_id: " << options.run_id << '\n';
    meta << "config: " << std::filesystem::absolute(options.config_path).string() << '\n';
    meta << "seed: " << seed << '\n';
    meta << "frames: " << trajectory.size() << '\n';
    meta << "seconds_per_frame: " << seconds_per_frame << '\n';
    meta << "reprojection_error: " << slam.reprojection_error() << '\n';

    write_point_cloud(run_dir / (options.sequence + ".ply"), slam);

    std::cout << "Trajectory written to: " << trajectory_path.string() << '\n';
    return true;
}
...
int run(const Options& options)
{
    ...
    if (!std::isfinite(fps) || fps <= 0.0F) {
        std::cerr << "A positive fps is required for temporal motion constraints" << '\n';
        return 1;
    }
    ...
    if (options.headless) {
        while (slam.step()) {
            if (options.last_frame > 0 && slam.frame().index() >= options.last_frame) {
                std::cout << "Stopping at frame " << slam.frame().index() << '\n';
                break;
            }
        }
    } else {
